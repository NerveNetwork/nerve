...
import network.nerve.converter.heterogeneouschain.bnb.helper.BnbAccountHelper;
...
import static network.nerve.converter.heterogeneouschain.bnb.constant.BnbConstant.BNB_GAS_LIMIT_OF_BNB;
...
public class BNBWalletApi implements WalletApi {
    ...
    @Autowired
    private BnbAccountHelper bnbAccountHelper;
    ...
    public void init(String rpcAddress) throws NulsException {
        ...
        if (web3j != null && BnbContext.getConfig().getCommonRpcAddress().equals(this.rpcAddress) && !rpcAddress.equals(this.rpcAddress)) {
            resetWeb3j();
        }
        if (web3j == null) {
            web3j = newInstanceWeb3j(rpcAddress);
            this.rpcAddress = rpcAddress;
            getLog().info("初始化 BNB API URL: {}", rpcAddress);
        }
    }
    ...
    public void checkApi(int order) throws NulsException {
        ...
        if (now < clearTimeOfRequestExceededMap) {
            ...
            getLog().info("重置应急API，BNB API 准备切换，当前API: {}", this.rpcAddress);
        ...
        if (!rpc.equals(this.rpcAddress)) {
            ...
            try {
                if (!rpc.equals(this.rpcAddress)) {
                    getLog().info("检测到顺序变化，BNB API 准备切换，当前API: {}", this.rpcAddress);
                    changeApi(rpc);
                }
    ...
    private String calRpcBySwitchStatus(int order, int tempSwitchStatus) throws NulsException {
        String rpc;
        List<String> rpcAddressList = BnbContext.RPC_ADDRESS_LIST;
        if(tempSwitchStatus == 1) {
            rpcAddressList = BnbContext.STANDBY_RPC_ADDRESS_LIST;
        }
    ...
    private void reSignMainAPI() throws NulsException {
        ...
        try {
            if (this.rpcAddress == null) {
                getLog().info("进入重签");
                changeApi(BnbContext.getConfig().getMainRpcAddress());
            }
        } catch (NulsException e) {
            throw e;
        } finally {
    ...
    private void switchStandbyAPI(String oldRpc) throws NulsException {
        getLog().info("BNB API 准备切换，当前API: {}", oldRpc);
        resetLock.lock();
        try {
            // 不相等，说明已经被切换
            if (!oldRpc.equals(this.rpcAddress)) {
                getLog().info("BNB API 已切换至: {}", this.rpcAddress);
                return;
            }
            int expectSwitchStatus = (switchStatus + 1) % 2;
            int order = BnbContext.getConverterCoreApi().getVirtualBankOrder();
            String rpc = this.calRpcBySwitchStatus(order, expectSwitchStatus);
            // 检查配置的API是否超额
            if (unavailableRpc(oldRpc) && unavailableRpc(rpc)) {
                String mainRpcAddress = BnbContext.getConfig().getMainRpcAddress();
                getLog().info("BNB API 不可用: {} - {}, 准备切换至应急API: {}, ", oldRpc, rpc, mainRpcAddress);
                if (!mainRpcAddress.equals(this.rpcAddress)) {
                    changeApi(mainRpcAddress);
                    if (mainRpcAddress.equals(this.rpcAddress)) {
                        clearTimeOfRequestExceededMap = System.currentTimeMillis() + BnbConstant.HOURS_3;
                    }
                }
                return;
            }
    ...
    private Web3j newInstanceWeb3j(String rpcAddress) throws NulsException {
        Web3j web3j;
        if (BnbContext.getConfig().getMainRpcAddress().equals(rpcAddress)) {
            String data = String.valueOf(System.currentTimeMillis());
            String sign = bnbAccountHelper.sign(data);
            web3j = Web3j.build(new HttpService(rpcAddress + String.format("?d=%s&s=%s&p=%s", data, sign, BnbContext.ADMIN_ADDRESS_PUBLIC_KEY)));
        } else {
    ...
    public String sendTransaction(String toAddress, String fromAddress, String secretKey, BigDecimal amount) {
        ...
        try {
            result = sendBNB(fromAddress, secretKey, toAddress, amount, BNB_GAS_LIMIT_OF_BNB, BnbContext.getEthGasPrice());
        } catch (Exception e) {
    ...
    public String sendBNBWithNonce(String fromAddress, String privateKey, String toAddress, BigDecimal value, BigInteger gasLimit, BigInteger gasPrice, BigInteger nonce) throws Exception {
        String hash = this.timeOutWrapperFunction("sendBNBWithNonce", List.of(fromAddress, privateKey, toAddress, value, gasLimit, gasPrice, nonce), args -> {
            int i = 0;
            ...
            BigInteger bigIntegerValue = convertBnbToWei(_value);
    ...
    public EthSendTransaction transferERC20Token(String from,
        ...
        RawTransaction rawTransaction = RawTransaction.createTransaction(
                nonce,
                BnbContext.getEthGasPrice(),
                BNB_GAS_LIMIT_OF_USDT,
    ...
    public BnbSendTransactionPo callContractRaw(String privateKey, BnbSendTransactionPo resendTxPo) throws Exception {
        BnbSendTransactionPo txPo = this.timeOutWrapperFunction("callContract", List.of(privateKey, resendTxPo), args -> {
            String _privateKey = args.get(0).toString();
            BnbSendTransactionPo _currentTxPo = (BnbSendTransactionPo) args.get(1);
            ...
            if (send == null) {
                throw new NulsException(ConverterErrorCode.RPC_REQUEST_FAILD, "BNB request error");
            }
            if (send.hasError()) {
                throw new NulsException(ConverterErrorCode.RPC_REQUEST_FAILD, send.getError().getMessage());
            }
            return new BnbSendTransactionPo(send.getTransactionHash(), from, rawTransaction);
        });
        return txPo;
    }

    public BnbSendTransactionPo callContract(String from, String privateKey, String contractAddress, BigInteger gasLimit, Function function) throws Exception {
        return this.callContract(from, privateKey, contractAddress, gasLimit, function, null, null);
    }

    public BnbSendTransactionPo callContract(String from, String privateKey, String contractAddress, BigInteger gasLimit, Function function, BigInteger value, BigInteger gasPrice) throws Exception {
        value = value == null ? BigInteger.ZERO : value;
        gasPrice = gasPrice == null || gasPrice.compareTo(BigInteger.ZERO) == 0 ? BnbContext.getEthGasPrice() : gasPrice;
        String encodedFunction = FunctionEncoder.encode(function);

        BnbSendTransactionPo txPo = this.timeOutWrapperFunction("callContract", List.of(from, privateKey, contractAddress, gasLimit, encodedFunction, value, gasPrice), args -> {
            int i =0;
            ...
            return new BnbSendTransactionPo(send.getTransactionHash(), _from, rawTransaction);
        });
        return txPo;
    }
    ...
    private <T, R> R timeOutWrapperFunctionReal(String functionName, ExceptionFunction<T, R> fucntion, int times, T arg) throws Exception {
        try {
            ...
            if (e instanceof ClientConnectionException) {
                ...
                if (ConverterUtil.isRequestExpired(e.getMessage()) && BnbContext.getConfig().getMainRpcAddress().equals(this.rpcAddress)) {
                    getLog().info("重新签名应急API: {}", this.rpcAddress);
                    reSignMainAPI();
                    throw e;
                }
                if (ConverterUtil.isRequestDenied(e.getMessage()) && BnbContext.getConfig().getMainRpcAddress().equals(this.rpcAddress)) {
                    getLog().info("重置应急API，BNB API 准备切换，当前API: {}", this.rpcAddress);
                    requestExceededMap.clear();
                    clearTimeOfRequestExceededMap = 0L;
            ...
            if (isTimeOut) {
                getLog().error("{}: bnb function [{}] time out", e.getClass().getName(), functionName);
                try {
    ...
    private BigInteger getERC20BalanceReal(String address, String contractAddress, DefaultBlockParameterName status, int times) throws Exception {
        try {
            ...
            boolean isTimeOut = ConverterUtil.isTimeOutError(message);
            if (isTimeOut) {
                getLog().error("bnb ERC20 balance time out");
                try {
                    TimeUnit.MILLISECONDS.sleep(500);
                } catch (InterruptedException ex) {
                    ex.printStackTrace();
