...
public class JanusWalletApiTest extends Base {
    ...
    public void transferERC20() throws Exception {
        ...
        if (token.hasError()) {
            System.err.println(String.format("【失败】向[%s]转账%s个ERC20(USDT), 交易hash: %s", to, tokenAmount, token.getError().getMessage()));
        } else {
        ...
        if (token.hasError()) {
            System.err.println(String.format("【失败】向[%s]转账%s个ERC20(USD18), 交易hash: %s", to, tokenAmount, token.getError().getMessage()));
        } else {
