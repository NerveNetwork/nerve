...
import network.nerve.converter.heterogeneouschain.ht.helper.HtAccountHelper;
...
import static network.nerve.converter.heterogeneouschain.ht.constant.HtConstant.HT_GAS_LIMIT_OF_HT;
import static network.nerve.converter.heterogeneouschain.ht.constant.HtConstant.HT_GAS_LIMIT_OF_USDT;
...
public class HtWalletApi implements WalletApi {
    ...
    @Autowired
    private HtAccountHelper htAccountHelper;
    ...
    public void init(String rpcAddress) throws NulsException {
        ...
        if (web3j != null && HtContext.getConfig().getCommonRpcAddress().equals(this.rpcAddress) && !rpcAddress.equals(this.rpcAddress)) {
            resetWeb3j();
        }
        if (web3j == null) {
            web3j = newInstanceWeb3j(rpcAddress);
            this.rpcAddress = rpcAddress;
            getLog().info("初始化 HT API URL: {}", rpcAddress);
        }
    }
    ...
    public void checkApi(int order) throws NulsException {
        ...
        if (now < clearTimeOfRequestExceededMap) {
            if (HtContext.getConfig().getMainRpcAddress().equals(this.rpcAddress)) {
            ...
            getLog().info("重置应急API，HT API 准备切换，当前API: {}", this.rpcAddress);
        ...
        if (!rpc.equals(this.rpcAddress)) {
            ...
            try {
                if (!rpc.equals(this.rpcAddress)) {
                    getLog().info("检测到顺序变化，HT API 准备切换，当前API: {}", this.rpcAddress);
                    changeApi(rpc);
                }
    ...
    private String calRpcBySwitchStatus(int order, int tempSwitchStatus) throws NulsException {
        String rpc;
        List<String> rpcAddressList = HtContext.RPC_ADDRESS_LIST;
        if(tempSwitchStatus == 1) {
            rpcAddressList = HtContext.STANDBY_RPC_ADDRESS_LIST;
        }
    ...
    private void reSignMainAPI() throws NulsException {
        ...
        try {
            if (this.rpcAddress == null) {
                getLog().info("进入重签");
                changeApi(HtContext.getConfig().getMainRpcAddress());
            }
        } catch (NulsException e) {
            throw e;
        } finally {
    ...
    private void switchStandbyAPI(String oldRpc) throws NulsException {
        getLog().info("HT API 准备切换，当前API: {}", oldRpc);
        resetLock.lock();
        try {
            // 不相等，说明已经被切换
            if (!oldRpc.equals(this.rpcAddress)) {
                getLog().info("HT API 已切换至: {}", this.rpcAddress);
                return;
            }
            int expectSwitchStatus = (switchStatus + 1) % 2;
            int order = HtContext.getConverterCoreApi().getVirtualBankOrder();
            String rpc = this.calRpcBySwitchStatus(order, expectSwitchStatus);
            // 检查配置的API是否超额
            if (unavailableRpc(oldRpc) && unavailableRpc(rpc)) {
                String mainRpcAddress = HtContext.getConfig().getMainRpcAddress();
                getLog().info("HT API 不可用: {} - {}, 准备切换至应急API: {}, ", oldRpc, rpc, mainRpcAddress);
                if (!mainRpcAddress.equals(this.rpcAddress)) {
                    changeApi(mainRpcAddress);
                    if (mainRpcAddress.equals(this.rpcAddress)) {
                        clearTimeOfRequestExceededMap = System.currentTimeMillis() + HtConstant.HOURS_3;
                    }
                }
                return;
            }
    ...
    private Web3j newInstanceWeb3j(String rpcAddress) throws NulsException {
        Web3j web3j;
        if (HtContext.getConfig().getMainRpcAddress().equals(rpcAddress)) {
            String data = String.valueOf(System.currentTimeMillis());
            String sign = htAccountHelper.sign(data);
            web3j = Web3j.build(new HttpService(rpcAddress + String.format("?d=%s&s=%s&p=%s", data, sign, HtContext.ADMIN_ADDRESS_PUBLIC_KEY)));
        } else {
    ...
    public String sendTransaction(String toAddress, String fromAddress, String secretKey, BigDecimal amount) {
        ...
        try {
            result = sendHT(fromAddress, secretKey, toAddress, amount, HT_GAS_LIMIT_OF_HT, HtContext.getEthGasPrice());
        } catch (Exception e) {
    ...
    public String sendHTWithNonce(String fromAddress, String privateKey, String toAddress, BigDecimal value, BigInteger gasLimit, BigInteger gasPrice, BigInteger nonce) throws Exception {
        String hash = this.timeOutWrapperFunction("sendHTWithNonce", List.of(fromAddress, privateKey, toAddress, value, gasLimit, gasPrice, nonce), args -> {
            int i = 0;
            ...
            BigInteger bigIntegerValue = convertHtToWei(_value);
    ...
    public EthSendTransaction transferERC20Token(String from,
        ...
        RawTransaction rawTransaction = RawTransaction.createTransaction(
                nonce,
                HtContext.getEthGasPrice(),
                HT_GAS_LIMIT_OF_USDT,
    ...
    public HtSendTransactionPo callContractRaw(String privateKey, HtSendTransactionPo resendTxPo) throws Exception {
        HtSendTransactionPo txPo = this.timeOutWrapperFunction("callContract", List.of(privateKey, resendTxPo), args -> {
            String _privateKey = args.get(0).toString();
            HtSendTransactionPo _currentTxPo = (HtSendTransactionPo) args.get(1);
            ...
            if (send == null) {
                throw new NulsException(ConverterErrorCode.RPC_REQUEST_FAILD, "HT request error");
            }
            if (send.hasError()) {
                throw new NulsException(ConverterErrorCode.RPC_REQUEST_FAILD, send.getError().getMessage());
            }
            return new HtSendTransactionPo(send.getTransactionHash(), from, rawTransaction);
        });
        return txPo;
    }

    public HtSendTransactionPo callContract(String from, String privateKey, String contractAddress, BigInteger gasLimit, Function function) throws Exception {
        return this.callContract(from, privateKey, contractAddress, gasLimit, function, null, null);
    }

    public HtSendTransactionPo callContract(String from, String privateKey, String contractAddress, BigInteger gasLimit, Function function, BigInteger value, BigInteger gasPrice) throws Exception {
        value = value == null ? BigInteger.ZERO : value;
        gasPrice = gasPrice == null || gasPrice.compareTo(BigInteger.ZERO) == 0 ? HtContext.getEthGasPrice() : gasPrice;
        String encodedFunction = FunctionEncoder.encode(function);

        HtSendTransactionPo txPo = this.timeOutWrapperFunction("callContract", List.of(from, privateKey, contractAddress, gasLimit, encodedFunction, value, gasPrice), args -> {
            int i =0;
            ...
            return new HtSendTransactionPo(send.getTransactionHash(), _from, rawTransaction);
        });
        return txPo;
    }
    ...
    private <T, R> R timeOutWrapperFunctionReal(String functionName, ExceptionFunction<T, R> fucntion, int times, T arg) throws Exception {
        try {
            ...
            if (e instanceof ClientConnectionException) {
                ...
                if (ConverterUtil.isRequestExpired(e.getMessage()) && HtContext.getConfig().getMainRpcAddress().equals(this.rpcAddress)) {
                    getLog().info("重新签名应急API: {}", this.rpcAddress);
                    reSignMainAPI();
                    throw e;
                }
                if (ConverterUtil.isRequestDenied(e.getMessage()) && HtContext.getConfig().getMainRpcAddress().equals(this.rpcAddress)) {
                    getLog().info("重置应急API，HT API 准备切换，当前API: {}", this.rpcAddress);
                    requestExceededMap.clear();
                    clearTimeOfRequestExceededMap = 0L;
            ...
            if (isTimeOut) {
                getLog().error("{}: ht function [{}] time out", e.getClass().getName(), functionName);
                try {
    ...
    private BigInteger getERC20BalanceReal(String address, String contractAddress, DefaultBlockParameterName status, int times) throws Exception {
        try {
            ...
            boolean isTimeOut = ConverterUtil.isTimeOutError(message);
            if (isTimeOut) {
                getLog().error("ht ERC20 balance time out");
                try {
                    TimeUnit.MILLISECONDS.sleep(500);
                } catch (InterruptedException ex) {
                    ex.printStackTrace();
