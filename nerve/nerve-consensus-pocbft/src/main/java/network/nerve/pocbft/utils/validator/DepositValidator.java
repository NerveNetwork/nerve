...
public class DepositValidator extends BaseValidator {
    ...
    public Result validate(Chain chain, Transaction tx) throws NulsException, IOException {
        ...
        //验证委托金额是否是否小于最小委托金额
        BigInteger realAmount = ConsensusAwardUtil.getRealAmount(chain.getChainId(), deposit.getDeposit(), deposit.getAssetChainId(), deposit.getAssetId(), tx.getTime() * 1000L);
        boolean pass = tx.getBlockHeight() > 0 && tx.getBlockHeight() < 1200000;
        if (!pass && realAmount.compareTo(chain.getConfig().getEntrustMin()) < 0) {
