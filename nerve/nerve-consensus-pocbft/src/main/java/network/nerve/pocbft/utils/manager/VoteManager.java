...
import network.nerve.pocbft.model.bo.vote.VoteStageData;
...
public class VoteManager {
    ...
    public static boolean statisticalResult(Chain chain, VoteMessage message, byte voteStage) throws Exception {
        ...
        //验证投票是否为共识节点投票
        MeetingRound meetingRound = roundManager.getRound(chain, VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex(), VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundStartTime());
        ...
        //如果不为当前轮次投票信息则直接 返回
        if (message.getRoundIndex() != VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex() || message.getPackingIndexOfRound() != VoteCache.CURRENT_BLOCK_VOTE_DATA.getPackingIndexOfRound()
                || message.getVoteStage() < VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteStage() || message.getHeight() != message.getHeight()) {
        ...
        if (VoteCache.CURRENT_BLOCK_VOTE_DATA.isFinished() || VoteCache.CURRENT_BLOCK_VOTE_DATA.getCurrentRoundData().isFinished()) {
            chain.getLogger().warn("The current round of voting has been confirmed to be completed,roundIndex:{},packIndexOfRound:{},voteRound:{},voteStage:{}", VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex(), VoteCache.CURRENT_BLOCK_VOTE_DATA.getPackingIndexOfRound(), VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteRound(), VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteStage());
            return true;
        }
        //处理当前投票轮次投票时间不同的投票
        long currentVoteRoundTime = VoteCache.CURRENT_BLOCK_VOTE_DATA.getCurrentRoundData().getTime();
        if(message.getTime() != currentVoteRoundTime){
            handleVoteBifurcate(chain, message, currentVoteRoundTime);
            return false;
        }
        VoteStageData stageData = VoteCache.CURRENT_BLOCK_VOTE_DATA.getCurrentVoteRoundStageData(voteStage);
        if(stageData.isFinished()){
            chain.getLogger().info("The result of the voting at this stage has been obtained" );
            return true;
        }
        //如果确认正常块/分叉且当前节点没有收到区块，则通知区块模块向该节点获取区块
        if(message.getVoteRound() > ConsensusConstant.VOTE_INIT_ROUND && NulsHash.EMPTY_NULS_HASH.equals(VoteCache.CURRENT_BLOCK_VOTE_DATA.getBlockHash()) && !NulsHash.EMPTY_NULS_HASH.equals(message.getBlockHash())){
            boolean bifurcate = message.getFirstHeader() != null && message.getSecondHeader() != null;
            NulsHash firstHash = bifurcate ? message.getFirstHeader().getHash() : message.getBlockHash();
            NulsHash secondHash = bifurcate ? message.getSecondHeader().getHash() : null;
            CallMethodUtils.noticeGetBlock(chain, message.getHeight(), message.getSendNode(), firstHash, secondHash);
        }
        ...
        if (voteTotalCount < VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinByzantineCount()) {
            return true;
        }
        VoteResultData voteResultData = null;
        if (maxRateCount >= VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinPassCount()) {
            ...
            chain.getLogger().info("本轮次本阶段投票得到结果，voteStage:{},isConfirmedEmpty:{},totalVoteCount:{},minByzantineCount:{},maxRateCount:{}", voteStage, item.isConfirmedEmpty(),voteTotalCount, VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinByzantineCount(),maxRateCount);
        } else {
            int otherCount = voteTotalCount - maxRateCount;
            if (maxRateCount >= VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinCoverCount() && otherCount >= VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinCoverCount()) {
                voteResultData = new VoteResultData();
                voteResultData.setResultSuccess(false);
                voteResultData.getVoteResultItemList().addAll(stageData.getItemMap().values());
                chain.getLogger().info("本轮次本阶段投票收集失败，voteStage:{},totalVoteCount:{},minByzantineCount:{},maxRateCount:{},otherCount:{}", voteStage,voteTotalCount, VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinByzantineCount(),maxRateCount,otherCount);
            }
        }
    ...
    private static void handleVoteBifurcate(Chain chain, VoteMessage message, long localVoteTime){
        ...
        Map<Byte, VoteResultData> currentBlockVoteResult =  VoteCache.CONFIRMED_VOTE_RESULT_MAP.get(message.getConsensusKey());
    ...
    public static boolean verifyVoteResult(Chain chain, VoteResultData voteResultData, MeetingRound round) {
        ...
        //如果本节点为共识节点则需要切换投票轮次
        if(VoteCache.CURRENT_BLOCK_VOTE_DATA != null && round.getMyMember() != null){
            chain.getLogger().debug("本节点为共识节点处理投票结果！" );
            handleVoteResult(chain, voteResultData);
        }else{
            chain.getLogger().debug("本节点不为共识节点，缓存投票结果，通知区块模块拜占庭完成！" );
            VoteCache.addVoteResult(chain, voteResultBasicInfo.getConsensusKey(), ConsensusConstant.FINAL_VOTE_ROUND_SIGN, voteResultData);
    ...
    public static void stopVote(Chain chain){
        try {
            ...
            chain.getLogger().info("共识变更，停止投票，清理投票轮系信息");
            try {
                while (!VoteCache.PRE_ROUND_CONFIRMED  && !VoteCache.CURRENT_ROUND_FINISH){
                    VoteCache.CURRENT_BLOCK_VOTE_DATA.getCurrentRoundData().getStageTwo().getVoteResult().complete(new VoteResultData());
            ...
            VoteCache.PRE_ROUND_CONFIRMED = true;
        } finally {
    ...
    private static synchronized void handleVoteResult(Chain chain, VoteResultData voteResultData) {
        ...
        if (resultTime == ConsensusConstant.PREVIOUS_ROUND) {
            ...
            chain.getLogger().debug("处理当前区块之后的区块的投票结果，当前投票信息：{}, 投票结果信息:{}", VoteCache.CURRENT_BLOCK_VOTE_DATA.toString(),voteResultItem.toString());
            //缓存投票结果
            String resultConsensusKey = voteResultItem.getConsensusKey();
            VoteCache.addVoteResult(chain, resultConsensusKey, ConsensusConstant.FINAL_VOTE_ROUND_SIGN, voteResultData);
    ...
    private static void handlePreRoundResult(Chain chain, VoteResultData voteResultData) {
        ...
        if (voteResultData.isResultSuccess()) {
            VoteCache.addVoteResult(chain, voteInfo.getConsensusKey(), ConsensusConstant.FINAL_VOTE_ROUND_SIGN, voteResultData);
            ...
            VoteCache.addVoteResult(chain, voteInfo.getConsensusKey(), voteRound, voteResultData);
    ...
    private static void handleCurrentRoundResult(Chain chain, VoteResultData voteResultData) {
         ...
         * */
        if (voteResultData.isResultSuccess()) {
            //如果当前投票轮次在本地已经得到最终结果，则不处理该投票结果
            if(VoteCache.CONFIRMED_VOTE_RESULT_MAP.containsKey(voteInfo.getConsensusKey())  && VoteCache.CONFIRMED_VOTE_RESULT_MAP.get(voteInfo.getConsensusKey()).containsKey(ConsensusConstant.FINAL_VOTE_ROUND_SIGN)){
                return;
            }
            VoteCache.addVoteResult(chain, voteInfo.getConsensusKey(), ConsensusConstant.FINAL_VOTE_ROUND_SIGN, voteResultData);
            if (voteResultData.isConfirmedEmpty()) {
    ...
    private static void handleCurrentBlockResult(Chain chain, VoteResultData voteResultData) {
        VoteCache.CURRENT_BLOCK_VOTE_DATA.setVoteResult(chain, voteResultData.getVoteResultItem().getVoteRound(), ConsensusConstant.VOTE_STAGE_TWO, voteResultData);
        VoteResultItem voteInfo = voteResultData.getVoteResultItem();
        byte voteRound = voteInfo.getVoteRound();
        byte voteStage = voteInfo.getVoteStage();
    ...
    public static void switchBlockVoteData(Chain chain, BlockHeader blockHeader) {
        try {
            ...
            boolean roundSwitched = roundIndex < VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex()
                    ||  roundIndex== VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex() && packIndex < VoteCache.CURRENT_BLOCK_VOTE_DATA.getPackingIndexOfRound();
            roundSwitched = roundSwitched && blockHeader.getHeight() <= VoteCache.CURRENT_BLOCK_VOTE_DATA.getHeight();
            if(roundSwitched){
            ...
            VoteResultData voteResultData = VoteCache.CONFIRMED_VOTE_RESULT_MAP.get(consensusKey).get(ConsensusConstant.FINAL_VOTE_ROUND_SIGN);
            //设置当前轮次投票结果
            VoteCache.CURRENT_BLOCK_VOTE_DATA.setVoteResult(chain, VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteRound(), ConsensusConstant.VOTE_STAGE_TWO, voteResultData);
            VoteResultItem voteResultItem = voteResultData.getVoteResultItem();
            ...
            try {
                ...
                while (chain.isNetworkState() && !VoteCache.PRE_ROUND_CONFIRMED && !VoteCache.CURRENT_ROUND_FINISH && chain.isPacker()){
                    chain.getLogger().debug("等待当前投票轮次结束,NetworkState:{},PRE_ROUND_CONFIRMED:{},CURRENT_ROUND_FINISH:{}",chain.isNetworkState(), VoteCache.PRE_ROUND_CONFIRMED, VoteCache.CURRENT_ROUND_FINISH);
                    TimeUnit.MILLISECONDS.sleep(100);
                }
            }catch (InterruptedException e){
                chain.getLogger().error(e);
            ...
            if (changeRound) {
                ...
                if(round.getMyMember() == null){
                    ...
                    VoteCache.clearVote(chain);
                    chain.getLogger().info("节点变更为非共识节点，清空投票伦次信息" );
                }
            }
            VoteCache.PRE_ROUND_CONFIRMED = true;
            chain.getLogger().debug("------------------区块投票信息切换完成-----------------" );
        } finally {
            VoteCache.VOTE_HANDOVER = false;
            switchVoteInfoLock.unlock();
        }
    }
    ...
    public static void switchVoteData(Chain chain, VoteResultData voteResultData, boolean setResult) {
        try {
            ...
            boolean isValidData = roundIndex == VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex()
                    && packIndex == VoteCache.CURRENT_BLOCK_VOTE_DATA.getPackingIndexOfRound() &&
                    (voteRound > VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteRound()
                            || (VoteCache.CONFIRMED_VOTE_RESULT_MAP.get(voteInfo.getConsensusKey()) == null
                            || VoteCache.CONFIRMED_VOTE_RESULT_MAP.get(voteInfo.getConsensusKey()).get(voteInfo.getVoteRound()) == null));
            ...
            //设置投票结果，并终止当前投票
            if(setResult){
                VoteCache.CURRENT_BLOCK_VOTE_DATA.setVoteResult(chain, voteRound, ConsensusConstant.VOTE_STAGE_TWO, voteResultData);
                if(voteRound != VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteRound()){
                    chain.getLogger().debug("如果不是当前投票轮次投票结果，则终止当前投票轮次" );
                    if(VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteStage() == ConsensusConstant.VOTE_STAGE_ONE){
                        VoteCache.CURRENT_BLOCK_VOTE_DATA.getCurrentRoundData().getStageOne().getVoteResult().complete(new VoteResultData());
                    }
                    VoteCache.CURRENT_BLOCK_VOTE_DATA.getCurrentRoundData().getStageTwo().getVoteResult().complete(new VoteResultData());
                }
            }
            ...
            long nextRoundIndex = roundIndex;
            int nextPackIndex = packIndex + 1;
            int agentCount = VoteCache.CURRENT_BLOCK_VOTE_DATA.getAgentCount();
            long nextHeight = VoteCache.CURRENT_BLOCK_VOTE_DATA.getHeight();
            boolean changeRound = false;
            ...
            try {
                while (chain.isNetworkState() && !VoteCache.PRE_ROUND_CONFIRMED  && !VoteCache.CURRENT_ROUND_FINISH){
                    chain.getLogger().debug("等待当前投票轮次结束" );
                    TimeUnit.MILLISECONDS.sleep(100);
                }
            }catch (InterruptedException e){
    ...
    private static void switchVoteData(Chain chain, long nextRoundIndex, int nextPackIndex, long nextVoteTime, int agentCount, long nextHeight, long roundStartTime) {
        ...
        if (VoteCache.FUTURE_VOTE_DATA.containsKey(voteKey)) {
            nextVoteData = VoteCache.FUTURE_VOTE_DATA.remove(voteKey);
        ...
        if (agentCount != VoteCache.CURRENT_BLOCK_VOTE_DATA.getAgentCount()) {
            int byzantineRate = chain.getConfig().getByzantineRate();
            int coverRate = ConsensusConstant.VALUE_OF_ONE_HUNDRED - byzantineRate;
            ...
            nextVoteData.setMinByzantineCount(VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinByzantineCount());
            nextVoteData.setMinPassCount(VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinPassCount());
            nextVoteData.setMinCoverCount(VoteCache.CURRENT_BLOCK_VOTE_DATA.getMinCoverCount());
        }
        //切换确认区块
        VoteCache.switchVoteDataMessage(chain, ConsensusConstant.VOTE_INIT_ROUND, nextVoteTime, nextVoteData);
    }
    ...
    public static void switchVoteRound(Chain chain, long roundIndex, int packIndex, byte voteRound, long voteTime, VoteResultData voteResultData){
        try {
             ...
             */
            boolean isValidData = roundIndex == VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex() && packIndex == VoteCache.CURRENT_BLOCK_VOTE_DATA.getPackingIndexOfRound();
            if(!isValidData){
                chain.getLogger().warn("Voting rounds switched");
                return;
            }
            String consensusKey = roundIndex + ConsensusConstant.SEPARATOR + packIndex;
            byte localVoteRound = VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteRound();
            if(voteRound > localVoteRound){
                ...
                if(VoteCache.CONFIRMED_VOTE_RESULT_MAP.get(consensusKey) != null && VoteCache.CONFIRMED_VOTE_RESULT_MAP.get(consensusKey).get(voteRound) != null){
    ...
    private static byte resultTime(VoteResultData voteResultData) {
        VoteResultItem voteInfo = voteResultData.getVoteResultItem();
        boolean isPreviousBlock = voteInfo.getRoundIndex() < VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex()
        ...
        if(voteInfo.getRoundIndex() == VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex() && voteInfo.getPackingIndexOfRound()  == VoteCache.CURRENT_BLOCK_VOTE_DATA.getPackingIndexOfRound() && voteInfo.getVoteRound() < VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteRound()){
            return ConsensusConstant.PREVIOUS_ROUND;
        }
        boolean isCurrentBlock = voteInfo.getRoundIndex() == VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex() && voteInfo.getPackingIndexOfRound()  == VoteCache.CURRENT_BLOCK_VOTE_DATA.getPackingIndexOfRound();
        boolean isCurrentRound = isCurrentBlock && voteInfo.getVoteRound()  == VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteRound();
        if (isCurrentRound) {
    ...
    private static void completeCurrentResult(Chain chain, long roundIndex, int packIndex, byte voteRound, byte voteStage, VoteResultData voteResultData){
        try {
            switchVoteInfoLock.lock();
            boolean isValidData = roundIndex == VoteCache.CURRENT_BLOCK_VOTE_DATA.getRoundIndex()
                    && packIndex == VoteCache.CURRENT_BLOCK_VOTE_DATA.getPackingIndexOfRound() && voteRound == VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteRound()
                    && voteStage == VoteCache.CURRENT_BLOCK_VOTE_DATA.getVoteStage();
            if(!isValidData){
                chain.getLogger().warn("Voting rounds switched");
                return;
            }
            VoteCache.CURRENT_BLOCK_VOTE_DATA.setVoteResult(chain, voteRound, voteStage, voteResultData);
        }finally {
            switchVoteInfoLock.unlock();
        }
    }
