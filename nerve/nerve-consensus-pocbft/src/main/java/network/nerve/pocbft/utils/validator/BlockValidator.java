...
import network.nerve.pocbft.model.bo.round.RoundValidResult;
import network.nerve.pocbft.model.bo.tx.txdata.Agent;
import network.nerve.pocbft.utils.compare.CoinFromComparator;
...
//                chain.getLogger().error("区块中：" + tx.getCoinData().length + ", 本地生成：" + coinBaseTransaction.getCoinData().length);
    ...
    private boolean verifyRedPunish(Chain chain, List<Transaction> redPunishTxs, String blockHeaderHash, Set<String> punishAddress, Block block) throws NulsException {
        ...
        for (Transaction redTx : redPunishTxs) {
            network.nerve.pocbft.model.bo.tx.txdata.RedPunishData data = new network.nerve.pocbft.model.bo.tx.txdata.RedPunishData();
     ...
     */
    private boolean verifyBifurcate(Chain chain, Transaction tx, network.nerve.pocbft.model.bo.tx.txdata.RedPunishData punishData) throws NulsException {
        /*
         红牌交易类型为连续分叉
     ...
     */
    private boolean coinDataValidate(Chain chain, Transaction tx, network.nerve.pocbft.model.bo.tx.txdata.RedPunishData punishData) throws NulsException {
