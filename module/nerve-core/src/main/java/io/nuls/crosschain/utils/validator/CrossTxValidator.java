...
public class CrossTxValidator {
    ...
    public boolean validateTx(Chain chain, Transaction tx, BlockHeader blockHeader) throws NulsException, IOException {
        ...
        if (chain.getChainId() == fromChainId) {
            ...
            ChainInfo chainInfo;
            //Transferring from the main network to a parallel chain, cutting the situation where the current chain is not the main network
            if (chain.getChainId() == toChainId && !config.isMainNet()) {
                ...
                //When switching back to the main network protocol, there is no need to reset ittxData
            ...
            //todo Currently, it only applies to cross chain communication between the main network and parallel chains. If there is cross chain communication between parallel chains, it needs to be calculated in other ways
        ...
        if (config.isMainNet()) {
            ...
            if(!coinDataManager.verifyCtxAsset(chain,tx)){
                chain.getLogger().info("Cross chain asset verification failed!The asset is not registered.");
                throw new NulsException(NulsCrossChainErrorCode.CROSS_ASSERT_VALID_ERROR);
            }
        }
        return true;
