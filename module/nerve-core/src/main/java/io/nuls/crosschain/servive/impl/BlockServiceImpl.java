...
import static io.nuls.crosschain.constant.ParamConstant.CHAIN_ID;
import static io.nuls.crosschain.constant.ParamConstant.NEW_BLOCK_HEIGHT;
...
//        chain.getLogger().info("Received block height update information, the latest block height is：{}", height);
    ...
    private Result paramValid(Map<String, Object> params) {
        ...
        try {
            ...
            /*
            Check if there are any round changes. If there are round changes, query the consensus module consensus node for any changes. If there are changes, create a validator change transaction(This operation needs to be done after the validator initializes the transaction)
            */
            if (chain.getVerifierList() != null && !chain.getVerifierList().isEmpty()) {
                ...
                if (agentChangeMap != null) {
                    ...
                    if (verifierChange) {
                        ...
                        chain.getCrossTxThreadPool().execute(new VerifierChangeTxHandler(chain, verifierChangeTx, blockHeader.getHeight(), blockHeader));
                    }
                }
            }
            chainManager.getChainHeaderMap().put(chainId, blockHeader);
