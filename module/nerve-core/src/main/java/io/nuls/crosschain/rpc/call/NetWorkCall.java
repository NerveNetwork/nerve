...
public class NetWorkCall {
    ...
    public static boolean register() {
        try {
            ...
            boolean success = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.NW.abbr, "nw_protocolRegister", map).isSuccess();
            while (!success) {
                Thread.sleep(1000L);
                success = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.NW.abbr, "nw_protocolRegister", map).isSuccess();
            }
            return true;
        } catch (Exception e) {
    ...
    public static boolean broadcast(int chainId, BaseMessage message, String excludeNodes, String command,boolean isCross) {
        try {
            ...
            Response cmdResp = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.NW.abbr, "nw_broadcast", params, NulsCrossChainConstant.RPC_TIME_OUT);
    ...
    public static int getAvailableNodeAmount(int chainId, boolean isCross) throws NulsException {
        ...
        try {
            Response callResp = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.NW.abbr, "nw_getChainConnectAmount", callParams);
    ...
    public static void activeCrossNet(int chainId,int maxOut,int maxIn,String seedIps )throws NulsException{
        ...
        try {
            Response callResp = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.NW.abbr, "nw_activeCross", callParams);
            if (!callResp.isSuccess()) {
