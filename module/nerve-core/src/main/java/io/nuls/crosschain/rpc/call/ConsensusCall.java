...
public class ConsensusCall {
    ...
    public static Map getPackerInfo(Chain chain) {
        try {
            ...
            Response cmdResp = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.CS.abbr, "cs_getPackerInfo", params);
    ...
    public static List<String> getWorkAgentList(Chain chain) {
        try {
            Map<String, Object> params = new HashMap(4);
            params.put(Constants.CHAIN_ID, chain.getChainId());
            Response cmdResp = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.CS.abbr, "cs_getAgentAddressList", params);
            if (!cmdResp.isSuccess()) {
                chain.getLogger().error("Packing state failed to send!");
            }
    ...
    public static Map getSeedNodeList(Chain chain) {
        try {
            Map<String, Object> params = new HashMap(4);
            params.put(Constants.CHAIN_ID, chain.getChainId());
            Response cmdResp = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.CS.abbr, "cs_getSeedNodeInfo", params);
            if (!cmdResp.isSuccess()) {
                chain.getLogger().error("Packing state failed to send!");
            }
    ...
    public static List<String> getRoundMemberList(Chain chain, BlockHeader blockHeader) {
        try {
            ...
            Response cmdResp = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.CS.abbr, "cs_getRoundMemberList", params);
            if (!cmdResp.isSuccess()) {
                chain.getLogger().error("Packing state failed to send!");
            }
    ...
    public static Map<String,List<String>> getAgentChangeInfo(Chain chain, byte[] lastRound, byte[] currentRound) {
        try {
            ...
            Response cmdResp = NerveCoreResponseMessageProcessor.requestAndResponse(ModuleE.CS.abbr, "cs_getAgentChangeInfo", params);
            if (!cmdResp.isSuccess()) {
                chain.getLogger().error("Packing state failed to send!");
            }
