...
//            }
            ...
            Result<BlockHeaderData> headerDataResult = blockService.getBlockHeaderByHeight(new GetBlockHeaderByHeightReq(transaction.getBlockHeight()));


            //发起拜占庭验证
            chain.getCrossTxThreadPool().execute(new CrossTxHandler(chain,  tx.getData(),null, syncStatus));
        }
    }
