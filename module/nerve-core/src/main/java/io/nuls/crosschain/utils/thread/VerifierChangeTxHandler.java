...
public class VerifierChangeTxHandler implements Runnable {
    ...
    public VerifierChangeTxHandler(Chain chain, Transaction transaction, long height, BlockHeader header) {
        this.chain = chain;
    ...
    private void verifierSplitHandle(Chain chain, Transaction transaction, long height, VerifierChangeData txData, boolean txChanged) {
        ...
        if (needSplit) {
            ...
            try {
                ...
                chain.getCrossTxThreadPool().execute(new VerifierChangeTxHandler(chain, firstTx, height, null));
                try {
                    TimeUnit.SECONDS.sleep(1);
                } catch (InterruptedException e) {
                    chain.getLogger().error(e);
                }
                chain.getCrossTxThreadPool().execute(new VerifierChangeTxHandler(chain, secondTx, height + 2, null));
            } catch (IOException e) {
                chain.getLogger().error(e);
            }
        } else {
            ...
            TxUtil.handleNewCtx(transaction, chain, header, txData.getCancelAgentList());
        }
    }
}
