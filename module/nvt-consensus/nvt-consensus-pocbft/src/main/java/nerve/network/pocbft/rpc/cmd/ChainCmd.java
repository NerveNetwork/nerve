...
import nerve.network.pocbft.model.dto.output.AccountConsensusInfoDTO;
import nerve.network.pocbft.model.dto.output.WholeNetConsensusInfoDTO;
import nerve.network.pocbft.service.ChainService;
...
public class ChainCmd extends BaseCmd {
    ...
    public Response getConsensusConfig(Map<String, Object> params) {
        Result<ChainConfig> result = service.getConsensusConfig(params);
        if (result.isFailed()) {
            return failed(result.getErrorCode());
        }
        return success(result.getData());
