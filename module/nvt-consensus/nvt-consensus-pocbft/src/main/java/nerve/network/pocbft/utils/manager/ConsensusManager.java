...
import nerve.network.pocbft.model.bo.BlockData;
import nerve.network.pocbft.model.bo.Chain;
import nerve.network.pocbft.model.bo.ChargeResultData;
import nerve.network.pocbft.model.bo.config.ConsensusChainConfig;
import nerve.network.pocbft.model.bo.consensus.AssembleBlockTxResult;
...
import nerve.network.pocbft.utils.RandomSeedUtils;
...
public class ConsensusManager {
    ...
    public static Block doPacking(Chain chain, MeetingMember member, MeetingRound round, long packingTime, boolean settleConsensusAward)throws Exception{
        ...
        */
        ...
        chain.getLogger().info("make block height:" + newBlock.getHeader().getHeight() + ",txCount: " + newBlock.getTxs().size() + " , block size: " + newBlock.size() + " , time:" + NulsDateUtils.convertDate(new Date(newBlock.getHeader().getTime() * 1000)) +  ",hash:" + newBlock.getHeader().getHash().toHex() + ",preHash:" + newBlock.getHeader().getPreHash().toHex());
        return newBlock;
    }
    ...
    private static AssembleBlockTxResult assembleBlockTxResult(Chain chain, BlockData bd, String packingAddressString, long packageHeight)throws Exception{
        ...
        BlockExtendsData bestExtendsData = chain.getNewestHeader().getExtendsData();
    ...
    private static void assembleSystemTx(Chain chain, BlockHeader bestBlock, List<Transaction> txList, MeetingMember member, MeetingRound round, long time, boolean settleConsensusAward)throws Exception{
        Transaction coinBaseTransaction = createCoinBaseTx(chain, member, txList, time, settleConsensusAward, true);
     ...
     * @return Transaction
     */
    public static Transaction createCoinBaseTx(Chain chain, MeetingMember member, List<Transaction> txList, long time, boolean settleConsensusAward, boolean isPack)throws Exception{
        ...
        */
        List<CoinTo> rewardList = calcReward(chain, txList, member, time, settleConsensusAward, isPack);
     ...
     * @param chain        chain info
     * @param txList       交易列表/transaction list
     * @param member         本地打包信息/local agent packing info
     * @param time           区块时间
     * @param settleConsensusAward  是否清算共识奖励
     * @param isPack                是打包途中还是验证中
     * @return List<CoinTo>
     */
    private static List<CoinTo> calcReward(Chain chain, List<Transaction> txList, MeetingMember member, long time, boolean settleConsensusAward, boolean isPack) throws  NulsException{
        /*
        资产与共识奖励键值对
        ...
        */
        ...
        if(!awardAssetMap.isEmpty()){
            for (Map.Entry<String, BigInteger> entry : awardAssetMap.entrySet()) {
                ...
                CoinTo coinTo = new CoinTo(member.getAgent().getRewardAddress(), Integer.valueOf(assetInfo[0]), Integer.valueOf(assetInfo[1]), entry.getValue(),0);
     ...
     */
    private static ChargeResultData getFee(Transaction tx, Chain chain) throws NulsException {
        CoinData coinData = new CoinData();
     ...
     * @return                 手续费大小
     * */
    public static BigInteger getFee(CoinData coinData , int assetChainId, int assetId){
