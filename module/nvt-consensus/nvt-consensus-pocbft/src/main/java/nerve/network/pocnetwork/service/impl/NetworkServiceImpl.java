package nerve.network.pocnetwork.service.impl;
...
public class NetworkServiceImpl implements NetworkService {
    ...
    public boolean sendShareMessage(int chainId, String peerNodeId, byte[] peerPubKey) {
        ...
        try {
            ...
            chain.getLogger().debug("===============shareMessage identityList={}", consensusShareMsg.getIdentityList().length);
    ...
    public boolean broadCastIdentityMsg(Chain chain) {
        try {
            ...
            if (null != consensusSeedPubKeyList) {
                chain.getLogger().debug("===============send broadCastIdentityMsg  consensusSeedPubKeyList={}", consensusSeedPubKeyList.size());
                for (ConsensusNet consensusNet : consensusSeedPubKeyList) {
                    if (!ArraysTool.arrayEquals(consensusNet.getPubKey(), consensusKeys.getPubKey()) && consensusNet.getPubKey().length > 0) {
                        try {
