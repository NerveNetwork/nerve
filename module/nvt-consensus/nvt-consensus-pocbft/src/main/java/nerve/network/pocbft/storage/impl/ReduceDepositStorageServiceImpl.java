...
import nerve.network.pocbft.storage.ReduceDepositStorageService;
...
public class ReduceDepositStorageServiceImpl implements ReduceDepositStorageService {
    ...
    public boolean save(ChangeAgentDepositPo po, int chainID) {
        ...
        try {
            ...
            return RocksDBService.put(ConsensusConstant.DB_NAME_REDUCE_DEPOSIT+ConsensusConstant.SEPARATOR +chainID,key,value);
        }catch (Exception e){
            Log.error(e);
            return  false;
        }
    ...
    public ChangeAgentDepositPo get(NulsHash txHash, int chainID) {
        try {
            byte[] key = txHash.getBytes();
            byte[] value = RocksDBService.get(ConsensusConstant.DB_NAME_REDUCE_DEPOSIT+ConsensusConstant.SEPARATOR +chainID,key);
            if(value == null){
                return  null;
            }
    ...
    public boolean delete(NulsHash txHash, int chainID) {
        try {
            byte[] key = txHash.getBytes();
            return RocksDBService.delete(ConsensusConstant.DB_NAME_REDUCE_DEPOSIT+ConsensusConstant.SEPARATOR +chainID,key);
        }catch (Exception e){
            Log.error(e);
            return  false;
        }
    ...
    public List<ChangeAgentDepositPo> getList(int chainID) throws NulsException {
        List<Entry<byte[], byte[]>> list = RocksDBService.entryList(ConsensusConstant.DB_NAME_REDUCE_DEPOSIT+ConsensusConstant.SEPARATOR +chainID);
