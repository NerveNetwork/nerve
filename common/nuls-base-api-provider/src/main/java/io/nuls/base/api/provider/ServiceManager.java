...
//                throw new RuntimeException("pub provider init fail, must be set chain-id in module.properties");
//            }
//            defaultChainId = Integer.parseInt(prop.getProperty("chain-id"));
//        } catch (IOException e) {
//            throw new RuntimeException("pub provider init fail, load module.properties fail");
//        }
        //2.加载服务提供类实例
                    ...
                    Arrays.stream(cls.getInterfaces()).forEach(intf->{
                        try {
                            if(cls.getSuperclass() == null && !cls.getSuperclass().equals(BaseService.class)){
                                throw new RuntimeException("pub provider init fail, Service must be extends BaseService");
                            }
                            ...
                            Log.error("pub provider init fail, service : {}",cls,e);
                            System.exit(0);
                        }
                    });
                }
